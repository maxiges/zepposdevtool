...
// DrawChartsForApp prepares and displays the plot widgets for the given app.
//
// If the plots haven't been initialized it creates them and configures layout
// and widgets. Afterwards it regenerates the plotted data and applies axis limits.
	...
	if currentMenu != MenuPlot {
		...
		MainUILayout = g.SplitLayout(g.DirectionHorizontal, &sashPos2,
			g.Layout{
				...
				g.Row(SelectMem1, SelectMem2, SelectMem3),
...
func GenerateDataForData(appName string) {
	...
	// Reset plot containers before populating them.
	plot1Data.Clear()
	plot2Data.Clear()
	plot3Data.Clear()
	...
	*MemorySizeUsedBar = *g.ProgressBar(float32(usedVal)/float32(dashboardData.Total)).Size(100, 20)
